...
function ShapePublic.vertexArray(Name: string, constructor: () -> ({Command: string, Data: {[any?]: any?}}), callback: (Window: Types.WindowRenderingContext, self: Types.RenderCommand) -> ()): nil
	if not Name or not constructor or not callback then
		error("Function 'shape.vertexArray' requires all parameters to be passed.\nNot all of the parameters were passed.\nDid you forget to pass them?")
	end
